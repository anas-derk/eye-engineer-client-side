...
export default function AddTerminology({
    setIsDisplayAddTerminologyBox,
    handleAddNewTerminology
}) {
    ...
    const handleClosePopupBox = () => {
        setIsDisplayAddTerminologyBox(false);
    }
    ...
    const addTerminology = async () => {
        try {
            ...
            if (Object.keys(errorsObject).length == 0) {
                setWaitMsg("Saving");
                const result = (await axios.post(`${process.env.BASE_API_URL}/terminologies/add?language=${i18n.language}`, {
                    term,
    ...
    return (
        <div className="add-link add-terminology popup-box">
            <div className="content-box d-flex align-items-center text-white flex-column p-4 text-center">
                <h2 className="mb-5 pb-3 border-bottom border-white">{t("Add New Terminology")}</h2>
                <section className="term mb-4 w-100">
                    <input
                        type="text"
                        placeholder={t("Please Enter Terminology")}
                        className={`form-control p-3 border-2 ${formValidationErrors["term"] ? "border-danger mb-3" : ""}`}
                        onChange={(e) => setTerm(e.target.value.trim())}
                    />
                    {formValidationErrors["term"] && <FormFieldErrorBox errorMsg={t(formValidationErrors["term"])} />}
                </section>
